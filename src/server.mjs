...
if (serverUrl.pathname === '/')
  ...
  // Respect the configured default page (e.g. login when disguiseFiles is enabled).
  app.get(serverUrl.pathname, (req, reply) => {
    const fileName = pages[pages.default] || pages.index;
    const type =
      supportedTypes[fileName.slice(fileName.lastIndexOf('.') + 1)] ||
      supportedTypes.default;
    ...
    reply
      .type(type)
      .send(tryReadFile('../views/dist/' + fileName, import.meta.url));
  });
}
